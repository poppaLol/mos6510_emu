...
fn when_acc_gt_target_addr_same_zero_flag_not_set() {
  ...
  assert!(!res.processor_status.intersects(Flags::Z_FLAG | Flags::N_FLAG));
...
fn when_acc_lt_target_addr_same_zero_flag_not_set() {
  ...
  assert!(!res.processor_status.intersects(Flags::Z_FLAG | Flags::C_FLAG));
  assert!(res.processor_status.contains(Flags::N_FLAG));
}
