...
fn when_bpl_branch_taken_cycles_increment_by_3() {
  ...
  mem.ram[0x4] = 0x9;
  let res = bpl(&mut mem, cpu);
  assert_eq!(res.program_counter, 0xE); //jump is taken - adds 9 to PC plus 2 from reading instructions
...
fn when_bpl_branch_taken_is_negative_program_counter_updated_correctly() {
  ...
  let res = bpl(&mut mem, cpu);
  assert_eq!(res.program_counter, 0x2FE) //jump is taken - minus 5 to PC plus 2 from reading instructions
}
...
fn when_bpl_branch_taken_across_page_boundary_cycles_inc_by_4() {
  ...
  let res = bpl(&mut mem, cpu);
  assert_eq!(res.program_counter, 0xEFFE); //jump is taken - minus 5 to PC plus 2 from reading instructions
...
fn when_bpl_branch_target_crosses_from_next_instruction_cycles_inc_by_4() {
  ...
  let res = bpl(&mut mem, cpu);
  assert_eq!(res.program_counter, 0x1100);
